...
<body class="bg-yellow-50">
    <div class="min-h-screen flex flex-col md:flex-row">
        ...
        <div id="sidebar" class="bg-black text-yellow-300 w-full md:w-64 py-6 flex flex-col fixed md:relative z-10 transform -translate-x-full md:translate-x-0 transition-transform duration-200 ease-in-out">
            ...
            <!-- Navigation Links -->
            <nav class="flex-1">
                <a href="{{ route('staff.dashboard') }}" class="flex items-center px-6 py-3 bg-yellow-600 text-black font-semibold">
                ...
                <a href="{{ route('staff.schedule') }}" class="flex items-center px-6 py-3 hover:bg-yellow-700 transition-colors duration-200">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
                    </svg>
                    Schedule
                </a>
                <a href="{{ route('staff.progress.index') }}" class="flex items-center px-6 py-3 hover:bg-yellow-700 transition-colors duration-200">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                    </svg>
                    Work Progress
                </a>
                <a href="{{ route('staff.profile') }}" class="flex items-center px-6 py-3 hover:bg-yellow-700 transition-colors duration-200">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                    </svg>
                    Profile
                </a>
                <a href="{{ route('staff.settings') }}" class="flex items-center px-6 py-3 hover:bg-yellow-700 transition-colors duration-200">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 w-full">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <!-- Staff Info -->
                <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 shadow-lg rounded-xl p-6 mb-8">
                    <h2 class="text-3xl font-bold text-black mb-2">Welcome back, {{ $staff->name }}</h2>
                    <p class="text-yellow-900 text-sm">{{ now()->format('l, F j, Y') }}</p>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mb-8">
                    <!-- Today's Attendance Card -->
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-yellow-200 hover:shadow-md transition-shadow duration-300">
                            ...
                            <div class="space-y-3">
                                ...
                                @else
                                    <form action="{{ route('staff.attendance.checkout') }}" method="POST">
                                        @csrf
                                        <button type="submit" class="w-full bg-yellow-600 text-black px-4 py-2 rounded-lg text-sm font-medium hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition-colors duration-200">Check Out</button>
                                    </form>
                                @endif
                            </div>
                        @else
                            <form action="{{ route('staff.attendance.checkin') }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full bg-yellow-600 text-black px-4 py-2 rounded-lg text-sm font-medium hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 transition-colors duration-200">Check In</button>
                            </form>
                        @endif
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div class="bg-white shadow-sm rounded-xl border border-yellow-200 overflow-hidden">
                        ...
                        <div class="divide-y divide-yellow-100 max-h-[400px] overflow-y-auto">
                            @if($upcomingAssignments->count() > 0)
                                @foreach($upcomingAssignments as $assignment)
                                    <div class="p-6 hover:bg-yellow-50 transition-colors duration-200">
                    ...
                    <div class="bg-white shadow-sm rounded-xl border border-yellow-200 overflow-hidden">
                        <div class="p-6 border-b border-yellow-100">
                            <div class="flex items-center justify-between">
                                ...
                                <a href="{{ route('staff.leave-requests.create') }}" class="bg-yellow-600 text-black px-4 py-2 rounded-lg text-sm hover:bg-yellow-700">New Request</a>
                            </div>
                        </div>
                        <div class="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
                            @if($leaveRequests->count() > 0)
                                ...
                                @endforeach
                            @else
                                <div class="p-6 text-center text-gray-500">No leave requests</div>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('click', function(event) {
            ...
            if (window.innerWidth < 768 && // Only on mobile
                ...
                !mobileMenuButton.contains(event.target) && // Click not on menu button
